...
#define __MULTIDICT_VALIDATION_CHECK(OBJECT, STATE, ON_FAIL)        \
    if (MultiDict_Check(((mod_state*)STATE), (self)) <= 0) {        \
        PyErr_SetString(PyExc_TypeError,                            \
                        #OBJECT " should be a MultiDict instance"); \
        return ON_FAIL;                                             \
    }
                     ...
                     PyObject* _default)
{
    ...
    if (md_set_default((MultiDictObject*)self, key, _default, &result) < 0) {
        return NULL;
    }
    return result;
}
